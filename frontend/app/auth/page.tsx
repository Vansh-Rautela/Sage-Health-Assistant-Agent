...
export default function DashboardPage() {
  ...
  const handleSelectSession = async (sessionId: string) => {
    ...
    if (session) {
      try {
        ...
        // A simple way to retrieve context: find the first user message
        const firstUserMessage = messages.find((m: Message) => m.role === 'user');
        let reportContext = {};
        if (firstUserMessage) {
            // This is a simplified context retrieval. A more robust solution would save context explicitly.
  ...
  const handleRiskScoreAnalysis = async (reportContext: any) => {
    try {
      ...
      setError("Could not generate risk scores.");
    }
  };
  ...
  const renderMainContent = () => {
    switch (viewState) {
      ...
      case 'chat':
        if (!currentSession) return <WelcomeState onNewSession={handleNewSession} />;
        return (
          <div className="flex-1 flex flex-col bg-gray-50 overflow-hidden">
            {/* The main scrollable area for chat history and risk scores */}
            <div className="flex-1 overflow-y-auto p-8 space-y-6">
              {riskData && <RiskScoreDisplay riskData={riskData} />}
              {/* Render messages from ChatView but without its input form */}
               {currentSession.messages.map((message, index) => (
                  <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-4xl ${message.role === 'user' ? 'neo-chat-user' : 'neo-chat-assistant'}`}>
                          <div dangerouslySetInnerHTML={{ __html: message.content.replace(/\n/g, '<br />') }} />
                      </div>
                  </div>
              ))}
            </div>
            {/* The single, correct chat input at the bottom */}
            <div className="border-t-4 border-black p-8 bg-white">
              <ChatView
                messages={[]} // Pass empty array as we are rendering messages above
